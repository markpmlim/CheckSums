...
extension Updateable {
    ...
    {
        _ = update(buffer: (data as NSData).bytes,  byteCount: size_t(data.count))
    ...
    {
        _ = update(buffer: byteArray, byteCount: size_t(byteArray.count))
        return self.status == Status.success ? self : nil
    }
    ///
    ...
    {
        _ = update(buffer: string, byteCount: size_t(string.lengthOfBytes(using: String.Encoding.utf8)))
        return self.status == Status.success ? self : nil
    }
}
